...
// include: C:\Users\domro\AppData\Local\Temp\tmp1kl19sn2.js
  ...
  (() => {
    ...
    async function loadPackage(metadata) {
      ...
      var PACKAGE_NAME = 'space_invaders.data';
      var REMOTE_PACKAGE_BASE = 'space_invaders.data';
...
Module['FS_createPath']("/", "Sounds", true, true);
    ...
    for (var file of metadata['files']) {
      ...
      async function processPackageData(arrayBuffer) {
        ...
        Module['removeRunDependency'](`fp ${name}`);
          ...
          Module['removeRunDependency']('datafile_space_invaders.data');
      }
      Module['addRunDependency']('datafile_space_invaders.data');
    ...
    loadPackage({"files": [{"filename": "/Graphics/alien_1.png", "start": 0, "end": 1827}, {"filename": "/Graphics/alien_2.png", "start": 1827, "end": 3661}, {"filename": "/Graphics/alien_3.png", "start": 3661, "end": 5587}, {"filename": "/Graphics/alien_ship.png", "start": 5587, "end": 7423}, {"filename": "/Graphics/spaceship.png", "start": 7423, "end": 9062}, {"filename": "/Sounds/explosion.ogg", "start": 9062, "end": 20037}, {"filename": "/Sounds/lazer.ogg", "start": 20037, "end": 36211}, {"filename": "/Sounds/music.ogg", "start": 36211, "end": 1225098}], "remote_package_size": 1225098});

  })();

// end include: C:\Users\domro\AppData\Local\Temp\tmp1kl19sn2.js
// include: C:\Users\domro\AppData\Local\Temp\tmpu_1kxo3y.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if ((typeof ENVIRONMENT_IS_WASM_WORKER != 'undefined' && ENVIRONMENT_IS_WASM_WORKER) || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD) || (typeof ENVIRONMENT_IS_AUDIO_WORKLET != 'undefined' && ENVIRONMENT_IS_AUDIO_WORKLET)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: C:\Users\domro\AppData\Local\Temp\tmpu_1kxo3y.js
// include: C:\Users\domro\AppData\Local\Temp\tmp2b0zx31o.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: C:\Users\domro\AppData\Local\Temp\tmp2b0zx31o.js
...
function findWasmBinary() {
  return locateFile('space_invaders.wasm');
}
...
var ASM_CONSTS = {
  149832: () => { if (document.fullscreenElement) return 1; },  
 149878: () => { return document.getElementById('canvas').width; },  
 149930: () => { return parseInt(document.getElementById('canvas').style.width); },  
 149998: () => { document.exitFullscreen(); },  
 150025: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 150098: () => { if (document.fullscreenElement) return 1; },  
 150144: () => { return document.getElementById('canvas').width; },  
 150196: () => { return screen.width; },  
 150221: () => { document.exitFullscreen(); },  
 150248: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 150381: () => { return window.innerWidth; },  
 150407: () => { return window.innerHeight; },  
 150434: () => { if (document.fullscreenElement) return 1; },  
 150480: () => { return document.getElementById('canvas').width; },  
 150532: () => { return parseInt(document.getElementById('canvas').style.width); },  
 150600: () => { if (document.fullscreenElement) return 1; },  
 150646: () => { return document.getElementById('canvas').width; },  
 150698: () => { return screen.width; },  
 150723: () => { return window.innerWidth; },  
 150749: () => { return window.innerHeight; },  
 150776: () => { if (document.fullscreenElement) return 1; },  
 150822: () => { return document.getElementById('canvas').width; },  
 150874: () => { return screen.width; },  
 150899: () => { document.exitFullscreen(); },  
 150926: () => { if (document.fullscreenElement) return 1; },  
 150972: () => { return document.getElementById('canvas').width; },  
 151024: () => { return parseInt(document.getElementById('canvas').style.width); },  
 151092: () => { document.exitFullscreen(); },  
 151119: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 151177: () => { return screen.width; },  
 151202: () => { return screen.height; },  
 151228: () => { return window.screenX; },  
 151255: () => { return window.screenY; },  
 151282: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 151335: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 151406: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 151463: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 151719: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 151790: () => { if (document.fullscreenElement) return 1; },  
 151836: () => { return window.innerWidth; },  
 151862: () => { return window.innerHeight; },  
 151889: () => { if (document.pointerLockElement) return 1; },  
 151936: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 154094: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 154258: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 154362: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 154533: ($0, $1, $2, $3, $4, $5) => { var deviceType = $0; var channels = $1; var sampleRate = $2; var bufferSize = $3; var pIntermediaryBuffer = $4; var pDevice = $5; if (typeof(window.miniaudio) === 'undefined') { return -1; } var device = {}; var audioContextOptions = {}; if (deviceType == window.miniaudio.device_type.playback && sampleRate != 0) { audioContextOptions.sampleRate = sampleRate; } device.webaudio = new (window.AudioContext || window.webkitAudioContext)(audioContextOptions); device.webaudio.suspend(); device.state = window.miniaudio.device_state.stopped; var channelCountIn = 0; var channelCountOut = channels; if (deviceType != window.miniaudio.device_type.playback) { channelCountIn = channels; } device.scriptNode = device.webaudio.createScriptProcessor(bufferSize, channelCountIn, channelCountOut); device.scriptNode.onaudioprocess = function(e) { if (device.intermediaryBufferView == null || device.intermediaryBufferView.length == 0) { device.intermediaryBufferView = new Float32Array(Module.HEAPF32.buffer, pIntermediaryBuffer, bufferSize * channels); } if (deviceType == miniaudio.device_type.capture || deviceType == miniaudio.device_type.duplex) { for (var iChannel = 0; iChannel < channels; iChannel += 1) { var inputBuffer = e.inputBuffer.getChannelData(iChannel); var intermediaryBuffer = device.intermediaryBufferView; for (var iFrame = 0; iFrame < bufferSize; iFrame += 1) { intermediaryBuffer[iFrame*channels + iChannel] = inputBuffer[iFrame]; } } _ma_device_process_pcm_frames_capture__webaudio(pDevice, bufferSize, pIntermediaryBuffer); } if (deviceType == miniaudio.device_type.playback || deviceType == miniaudio.device_type.duplex) { _ma_device_process_pcm_frames_playback__webaudio(pDevice, bufferSize, pIntermediaryBuffer); for (var iChannel = 0; iChannel < e.outputBuffer.numberOfChannels; ++iChannel) { var outputBuffer = e.outputBuffer.getChannelData(iChannel); var intermediaryBuffer = device.intermediaryBufferView; for (var iFrame = 0; iFrame < bufferSize; iFrame += 1) { outputBuffer[iFrame] = intermediaryBuffer[iFrame*channels + iChannel]; } } } else { for (var iChannel = 0; iChannel < e.outputBuffer.numberOfChannels; ++iChannel) { e.outputBuffer.getChannelData(iChannel).fill(0.0); } } }; if (deviceType == miniaudio.device_type.capture || deviceType == miniaudio.device_type.duplex) { navigator.mediaDevices.getUserMedia({audio:true, video:false}) .then(function(stream) { device.streamNode = device.webaudio.createMediaStreamSource(stream); device.streamNode.connect(device.scriptNode); device.scriptNode.connect(device.webaudio.destination); }) .catch(function(error) { console.log("Failed to get user media: " + error); }); } if (deviceType == miniaudio.device_type.playback) { device.scriptNode.connect(device.webaudio.destination); } device.pDevice = pDevice; return miniaudio.track_device(device); },  
 157361: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 157427: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 157820: ($0) => { miniaudio.untrack_device_by_index($0); },  
 157863: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 157988: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
