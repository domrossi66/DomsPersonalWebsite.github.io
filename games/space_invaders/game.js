...
// include: C:\Users\domro\AppData\Local\Temp\tmp5b9uf4i0.js
...
// end include: C:\Users\domro\AppData\Local\Temp\tmp5b9uf4i0.js
// include: C:\Users\domro\AppData\Local\Temp\tmpg2b35p3b.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if ((typeof ENVIRONMENT_IS_WASM_WORKER != 'undefined' && ENVIRONMENT_IS_WASM_WORKER) || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD) || (typeof ENVIRONMENT_IS_AUDIO_WORKLET != 'undefined' && ENVIRONMENT_IS_AUDIO_WORKLET)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: C:\Users\domro\AppData\Local\Temp\tmpg2b35p3b.js
// include: C:\Users\domro\AppData\Local\Temp\tmpa5lbofno.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: C:\Users\domro\AppData\Local\Temp\tmpa5lbofno.js
...
var ASM_CONSTS = {
  151576: () => { if (document.fullscreenElement) return 1; },  
 151622: () => { return document.getElementById('canvas').width; },  
 151674: () => { return parseInt(document.getElementById('canvas').style.width); },  
 151742: () => { document.exitFullscreen(); },  
 151769: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 151842: () => { if (document.fullscreenElement) return 1; },  
 151888: () => { return document.getElementById('canvas').width; },  
 151940: () => { return screen.width; },  
 151965: () => { document.exitFullscreen(); },  
 151992: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 152125: () => { return window.innerWidth; },  
 152151: () => { return window.innerHeight; },  
 152178: () => { if (document.fullscreenElement) return 1; },  
 152224: () => { return document.getElementById('canvas').width; },  
 152276: () => { return parseInt(document.getElementById('canvas').style.width); },  
 152344: () => { if (document.fullscreenElement) return 1; },  
 152390: () => { return document.getElementById('canvas').width; },  
 152442: () => { return screen.width; },  
 152467: () => { return window.innerWidth; },  
 152493: () => { return window.innerHeight; },  
 152520: () => { if (document.fullscreenElement) return 1; },  
 152566: () => { return document.getElementById('canvas').width; },  
 152618: () => { return screen.width; },  
 152643: () => { document.exitFullscreen(); },  
 152670: () => { if (document.fullscreenElement) return 1; },  
 152716: () => { return document.getElementById('canvas').width; },  
 152768: () => { return parseInt(document.getElementById('canvas').style.width); },  
 152836: () => { document.exitFullscreen(); },  
 152863: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 152921: () => { return screen.width; },  
 152946: () => { return screen.height; },  
 152972: () => { return window.screenX; },  
 152999: () => { return window.screenY; },  
 153026: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 153079: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 153150: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 153207: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 153463: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 153534: () => { if (document.fullscreenElement) return 1; },  
 153580: () => { return window.innerWidth; },  
 153606: () => { return window.innerHeight; },  
 153633: () => { if (document.pointerLockElement) return 1; },  
 153680: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 155838: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 156002: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 156106: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 159105: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 159171: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 159564: ($0) => { miniaudio.untrack_device_by_index($0); },  
 159607: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 159732: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
